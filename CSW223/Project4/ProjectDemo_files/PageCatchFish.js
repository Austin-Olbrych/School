...
*/
function PageCatchFish(pageName, animationList, optionList)
{
	...
	this.doMouseUp = function doMouseUp(point)
	{
		if (this.userSelectedObject != null)
		{
			...
			var lastObject = _animationList[1];
			var nameOfLastObject = lastObject.getName();
			//alert("nameOfLastObject "+nameOfLastObject);
			
			if (nameOfLastObject == "bucket")
			{
				if (lastObject.hitTest(point))
				{
					this.userSelectedObject.canDrag = false;
					_numFishDropped++;
					if(_numFishDropped == 3){
						alert('You caught three fish!');
